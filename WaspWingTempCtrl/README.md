...
程序根据电池温度和 CPU 温度综合决策，输出一个 **-3 ~ 10** 的档位。每个档位映射到具体的散热器控制参数：

| 档位 | 模式 | 目标温度 | 风扇转速 | 制冷片 | 说明 |
|------|------|---------|---------|--------|------|
| 10 | 智能温控(mode=0) | 10°C | 6000RPM | 自动 | 峰值制冷 |
| 9 | 智能温控(mode=0) | 11°C | 6000RPM | 自动 | |
| 8 | 智能温控(mode=0) | 12°C | 6000RPM | 自动 | |
| 7 | 智能温控(mode=0) | 13°C | 6000RPM | 自动 | |
| **6** | **智能温控(mode=0)** | **14°C** | **6000RPM** | **自动** | **满速临界** |
| 5 | 智能温控(mode=0) | 15°C | 5600RPM | 自动 | |
| 4 | 智能温控(mode=0) | 16°C | 5200RPM | 自动 | |
| 3 | 智能温控(mode=0) | 17°C | 4800RPM | 自动 | |
| 2 | 智能温控(mode=0) | 18°C | 4400RPM | 自动 | |
| 1 | 智能温控(mode=0) | 19°C | 4000RPM | 自动 | |
| **0** | **智能温控(mode=0)** | **20°C** | **3600RPM** | **自动** | **最弱智能** |
| -1 | 固定功率(mode=1) | — | 3200RPM | 60 | 低功耗 |
| -2 | 固定功率(mode=1) | — | 2800RPM | 35 | 更低功耗 |
| -3 | 固定功率(mode=1) | — | 2400RPM | 20 | 最低待机 |
...
| 参数 | 对应档位含义 |
|------|------------|
| `mode` | 0=智能温控（档位≥0），1=固定功率（档位<0） |
| `targetTemperature` | 智能温控的目标温度，`20 - 档位`，范围 5~35°C |
| `windLevelOverclock` | 固定功率模式下**风扇转速** (RPM)，范围 2000~6000 |
| `coldLevelOverclock` | 固定功率模式下**制冷片强度**，范围 1~230（实际最大有效 194） |
| `windLevel` | 智能温控模式下**风扇转速上限** (RPM) |
...
>85°C → 紧急 3 级 → 强制最低 8 档
>75°C → 紧急 2 级 → 强制最低 6 档
>65°C → 紧急 1 级 → 强制最低 4 档
...
WaspWingTempCtrl/
├── 智能控制/
│   ├── tempctrl.c                 # C 语言智能温控守护程序源码
│   ├── 新建 文本文档.txt           # 原始算法设计草稿
│   └── 新建 文本文档 (2).txt       # service.sh 模板（Magisk 保活）
├── app/
